...
//    }
     ...
     * @return list of object
     */
//    public static List<Contacts.People> getPeopleData(Context ctx) {
...
//     * @return list of object
//     */
    public static List<Image> getImageData(Context ctx) {
       ...
       try
        {
            ...
            SQLiteDatabase db = ctx.openOrCreateDatabase("lakshay.db", SQLiteDatabase.CREATE_IF_NECESSARY, null);
            String sql="SELECT Student.title,Student.description,Student.path,Student3.tag,Student.id from Student INNER JOIN Student3 ON Student.tagid=Student3.id  ";
...
//               hm.put("k5",c.getString(5));
//               hm.put("k6",c.getString(6));
//               hm.put("k7",c.getString(7));
                ar.add(hm);
                c.moveToNext();
...
//            ListAdapter listAdapter=new SimpleAdapter(this,ar,R.layout.layout,new String[]{"k1","k2","k3"},new int[]{R.id.t1,R.id.t2,R.id.t3});
//            li.setAdapter(listAdapter);
        }
        catch(Exception ex)
        {
           // Toast.makeText(ctx,"Error:"+ex.toString(),Toast.LENGTH_LONG).show();
            Log.d("Error","Database Problem");
        }
...
//        String date_arr[] = ctx.getResources().getStringArray(R.array.general_date);
        for (int i = 0; i <ar.size(); i++) {
            ...
            obj.name = ar.get(i).get("k1");
            obj.brief = ar.get(i).get("k2");

            obj.tags=ar.get(i).get("k4");


            obj.path=ar.get(i).get("k3");
            obj.id=Integer.parseInt(Objects.requireNonNull(ar.get(i).get("k5")));

           // obj.imageDrw = ctx.getResources().getDrawable(obj.image);
            obj.imageDrw=new BitmapDrawable(BitmapFactory.decodeFile(ar.get(i).get("k3")));
                    //new BitmapDrawable(new DataBaseHandler(ctx).getImage(1));
            items.add(obj);
        }
        //Collections.shuffle(items);
        return items;
